...
public class TC001_AccountRegistrationTest extends BaseClass {
	...
	@Test(priority = 2)
	public void Verify_UserAccountInformationDetails() {
		// Part 2 :
		logger.info("Verify Enter Account information Message");
		SignUpPage signup = new SignUpPage(driver);

		if (signup.get_msg_EnterAccountInformation().equalsIgnoreCase("Enter Account Information")) {
			...
			signup.send_LastName(p.getProperty("LastName"));
			logger.info("Enter Company");
			signup.send_Company(p.getProperty("Company"));
			...
			signup.send_City(p.getProperty("City"));
			...
			if (signup.get_msg_AccountCreated().equals("ACCOUNT CREATED!")) {
				...
				Assert.assertEquals(signup.get_msg_Congratulatons(),
				...
				Assert.assertEquals(signup.get_msg_AccountDeleted(), "ACCOUNT DELETED!",
						"ACCOUNT DELETED! is not displayed");
